import React, { PureComponent } from 'react';
import {Jumbotron, Thumbnail} from 'react-bootstrap'
import axios from 'axios'
import './Detail.css';
import store from '../store'
import {BrowserRouter as Router} from 'react-router-dom'

class Detail extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      data: store.getState().detail,
      // location: store.getState().detail.location,
      // user_rating: store.getState().detail.user_rating
    }
    store.subscribe ( () => {
      this.setState({
        data: store.getState().detail,
      })
    })
  }

  componentDidMount() {
    let category = this.props.match.params.category
    this.fetchRestaurantData(category);
  }

  fetchRestaurantData(category) {
    axios.get(`https://developers.zomato.com/api/v2.1/search?category=${category}&sort=cost&order=asc`,{headers: { 'user-key' : '4fea6930ef5dd45a54f63284d8d71508'}})
      .then(response => {
          // console.log(response.data.restaurants)
          let results = response.data.restaurants
          results.map(data => {
            if (data.restaurant.id === this.props.match.params.id) {
              console.log(data)
              store.dispatch({
                type: 'GET_DETAIL_RESTAURANT',
                payload: data.restaurant
              })
              // this.setState({data: data.restaurant})
              // this.setState({location: data.restaurant.location})
              // this.setState({user_rating: data.restaurant.user_rating})
            }
          })
          // console.log(this.state.data)
      })
      .catch(err => console.log(err));
  }
    
    render() {
        return (
          <Router>
            <Jumbotron>
              <Thumbnail src={this.state.data.featured_image || 'data:image/png;base64,' } alt="242x200"/>
              <h1>{this.state.data.name}</h1>
              <p className="restaurant-name">Price for Two: {this.state.data.average_cost_for_two} {this.state.data.currency}</p>
              {/* <p>{this.state.location.address}</p>
              <p>Votes: {this.user_rating.votes}</p> */}
            </Jumbotron>
          </Router>
        );
    }
}

export default Detail;